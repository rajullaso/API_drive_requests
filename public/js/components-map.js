function createBasicMap(mapId, lat, long, markerImage) {
    ...
    if ($('#' + mapId).length > 0) {
        ...
        if (mapMarker) {
            ...
            marker.bindPopup("<div class='p-4'><h5>Info Window Content</h5><p class='mb-0'>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.</p></div>", {
                minwidth: 200,
                maxWidth: 600,
...
function createAdvancedMap(options) {
    ...
    var defaults = {
        ...
        markerPathHighlight: 'img/map-marker-default.png',
    ...
    */
    ...
    function getPopupContent(properties) {
        ...
        if (properties.address) {
            var address = '<p class="text-muted mb-1"><i class="fa fa-map-marker fa-fw text-dark mr-2"></i>' + properties.address + '</p>'
        } else {
            address = ''
        }
        if (properties.email) {
            var email = '<p class="text-muted mb-1"><i class="fa fa-envelope-open fa-fw text-dark mr-2"></i><a href="mailto:' + properties.email + '" class="text-muted">' + properties.email + '</a></p>'
        } else {
            email = ''
        }
        if (properties.phone) {
